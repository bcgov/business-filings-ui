...
describe('Auth API Mixin', () => {
  ...
  it('fetches authorizations correctly', async () => {
    ...
    // mock endpoint
    get.withArgs('CP1234567/authorizations')
  ...
  it('fetches business info correctly', async () => {
    ...
    // mock endpoint
    get.withArgs('CP1234567')
      .returns(new Promise((resolve) => resolve({ data: BUSINESS_INFO })))

    // call method
    const businessInfo = await vm.fetchBusinessInfo('CP1234567')

    // verify data
    expect(businessInfo).toEqual({ data: BUSINESS_INFO })
  })
})
