...
export default class EnumMixin extends Vue {
  ...
  isStatusPending (item: any): boolean {
    return (
      (item.status === 'PENDING_ALTERATION') || // TODO: remove this workaround when API stops sending this
      (item.status === 'PENDING_CORRECTION') || // TODO: remove this workaround when API stops sending this
      (item.status === FilingStatus.PENDING)
    )
  }
