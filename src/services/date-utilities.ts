...
export default class DateUtilities {
   ...
   */
  static dateToYyyyMmDd (date: Date): IsoDatePacific {
    // safety check
   ...
   */
  static formatYyyyMmDd (dateStr: IsoDatePacific): string {
    // safety checks
    if (!dateStr) return null
    if (dateStr.length !== 10) return null
   ...
   */
  static dateToApi (date: Date): ApiDateTimeUtc {
    // safety check
    if (!isDate(date) || isNaN(date.getTime())) return null
   ...
   */
  static apiToPacificDateTime (dateTimeString: ApiDateTimeUtc, longMonth = false): string {
    if (!dateTimeString) return null // safety check
   ...
   */
  static apiToYyyyMmDd (dateTimeString: ApiDateTimeUtc): IsoDatePacific {
    const date = this.apiToDate(dateTimeString)
   ...
   */
  static yyyyMmDdToApi (date: IsoDatePacific): ApiDateTimeUtc {
    // safety check
   ...
   */
  static compareYyyyMmDd (date1: ApiDateTimeUtc, date2: ApiDateTimeUtc, operator: string): boolean {
    // safety check
   ...
   */
  static earliestYyyyMmDd (date1: ApiDateTimeUtc, date2: ApiDateTimeUtc): string {
    // safety check
   ...
   * @param date2 second date
   */
  static latestYyyyMmDd (date1: ApiDateTimeUtc, date2: ApiDateTimeUtc): string {
    // safety check
    if (!date1 && !date2) return null
