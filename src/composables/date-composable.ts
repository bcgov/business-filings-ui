...
/** Composable that provides some useful date utilities. */
export const DateComposable = () => {
  const getCurrentJsDate = computed(() => store.getters['getCurrentJsDate'] as Date)
   ...
   */
  const yyyyMmDdToDate = (dateStr: IsoDatePacific): Date => {
    // safety checks
   ...
   */
  const dateToYyyyMmDd = (date: Date): IsoDatePacific => {
    // safety check
   ...
   */
  const formatYyyyMmDd = (dateStr: IsoDatePacific): string => {
    // safety checks
    if (!dateStr) return null
    if (dateStr.length !== 10) return null
   ...
   */
  const dateToPacificTime = (date: Date): string => {
    // safety check
    if (!isDate(date) || isNaN(date.getTime())) return null
   ...
   */
  const dateToPacificDateTime = (date: Date): string => {
    // safety check
    if (!isDate(date) || isNaN(date.getTime())) return null
   ...
   */
  const dateToApi = (date: Date): ApiDateTimeUtc => {
    // safety check
    if (!isDate(date) || isNaN(date.getTime())) return null
   ...
   */
  const apiToDate = (dateTimeString: ApiDateTimeUtc): Date => {
   ...
   */
  const apiToPacificDate = (dateTimeString: ApiDateTimeUtc): string => {
    if (!dateTimeString) return null // safety check

    const date = apiToDate(dateTimeString)

    return dateToPacificDate(date)
  }
   ...
   */
  const apiToPacificDateTime = (dateTimeString: ApiDateTimeUtc, longMonth = false): string => {
    if (!dateTimeString) return null // safety check
   ...
   */
  const apiToUtcString = (dateTimeString: ApiDateTimeUtc): string => {
    if (!dateTimeString) return null // safety check
   ...
   */
  const apiToYyyyMmDd = (dateTimeString: ApiDateTimeUtc): IsoDatePacific => {
    const date = apiToDate(dateTimeString)
   ...
   */
  const yyyyMmDdToApi = (date: IsoDatePacific): ApiDateTimeUtc => {
    // safety check
   ...
   */
  const compareYyyyMmDd = (date1: ApiDateTimeUtc, date2: ApiDateTimeUtc, operator: string): boolean => {
    // safety check
   ...
   */
  const earliestYyyyMmDd = (date1: ApiDateTimeUtc, date2: ApiDateTimeUtc): string => {
    // safety check
    ...
    return (compareYyyyMmDd(date1, date2, '<') ? date1 : date2)
  }
   ...
   * @param date2 second date
   */
  const latestYyyyMmDd = (date1: ApiDateTimeUtc, date2: ApiDateTimeUtc): string => {
    // safety check
    if (!date1 && !date2) return null
  ...
  const daysFromToday = (date: Date): number => {
    ...
    const today = getCurrentJsDate.value
  ...
  return {
    ...
    formatYyyyMmDd,
    dateToPacificDate,
    dateToPacificDateTime,
    dateToApi,
    apiToDate,
    apiToPacificDate,
    apiToPacificDateTime,
    apiToYyyyMmDd,
    yyyyMmDdToApi,
    compareYyyyMmDd,
    earliestYyyyMmDd,
